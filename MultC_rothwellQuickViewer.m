...
recent_file=char(d(y(end)).name);  %rack_test_2013xxx


load(recent_file);

% fname = sprintf('20140115_170146'); 
...
t_bic= data_bic(:,1);
t_tri= data_tri(:,1);
angle_bic = data_bic(:,2);
angle_tri = data_tri(:,2);
length_bic = data_bic(:,3);
length_tri = data_tri(:,3);
...
linearV_bic = data_bic(:,7);
linearV_tri = data_tri(:,7);
...
%% EMG processing
Fe=1024;%1024; %Samling frequency
Fc_lpf=50.0; % Cut-off frequency  % was 450
Fc_hpf=0.5;   % was 0.5 
N=3; % Filter Order
...
% 
% % axis off;  
% hLine1 = line(t_bic(start:last), angle_bic(start:last));
...
% subplot(n, 1, 1);
% 
% plot(t_bic, angle_bic, 'LineWidth',2, 'color', 'black');    
% %     [pks, locs] = findpeaks(length_bic_cut);
% 
% axis off
% 
% subplot(n, 1, 2); 
% plot(t_bic, EMG_bic, 'LineWidth',2, 'color', 'black');
% axis off
% 
% subplot(n, 1, 3); 
% plot(t_tri, EMG_tri, 'LineWidth',2, 'color', 'black');
% axis off
% 
% hfig2 = figure(2);
% n=3;
...
%%
hfig2  = figure(2); 
last_bic = min(length(t_bic), 22000) 
last_tri = min(length(t_tri), 22000) 
% last = 635
...
subplot(n, 1, 1);
hLine4 = line(t_bic(start:last_bic), angle_bic(start:last_bic));
set(hLine4                        , ...
  'LineStyle'       , '-'        , ...
  'LineWidth'       , 3           , ... 
  'Color'           , 'black'  );
...
subplot (n,1, 2);
hLine5 = line(t_bic(start:last_bic), linearV_bic(start:last_bic));
set(hLine5                        , ...
  'LineStyle'       , '-'         , ...
  'LineWidth'       , 1           , ...   
  'Color'           , 'black'  );
...
hXLabel = xlabel('time (s)');
hYLabel = ylabel('bic vel ');

subplot (n,1, 3);
hLine5 = line(t_bic(start:last_bic), abs(EMG_bic(start:last_bic)));
set(hLine5                        , ...
  'LineStyle'       , '-'         , ...
  'LineWidth'       , 1           , ...   
  'Color'           , 'black'  );
...
subplot (n,1, 4);
hLine6 = line(t_tri(start:last_tri), abs(EMG_tri(start:last_tri)));
set(hLine6                        , ...
  'LineStyle'       , '-'         , ...
  'LineWidth'       , 1           , ...   
  'Color'           , 'black');
...
% print(hfig, '-deps', [recent_file, '_perturb_bic']);
print(hfig2, '-deps', [recent_file, '_perturb_tri']);
